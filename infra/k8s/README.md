...
Kubernetes manifests or Helm-style overlays will live here for local Kubernetes first and Amazon EKS production targeting later. EKS is the intended production Kubernetes platform, but this directory is docs-only and local-only for now; no production manifests, deploy automation, or approved AWS deployment workflow is implemented here yet.
...
2. Local dependencies or bindings for MySQL and Redis that match the existing Docker Compose development assumptions.
