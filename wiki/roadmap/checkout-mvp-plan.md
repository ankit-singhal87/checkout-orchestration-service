...
- `PUT /api/checkout/state`: p95 under `500ms`, verifies a tenant-visible cart/session and initializes/resumes state.
- `GET /api/checkout/state`: p95 under `500ms`, with MySQL as source of truth and Redis read-through cache later when needed.
- Address/shipping/payment selection endpoints under `/api/checkout/state/*`: p95 under `700ms`, because they can invalidate dependent state.
- Basket quantity/voucher updates: p95 under `700ms` after those APIs are wired, with stock and pricing recalculation bounded by service deadlines.
- `POST /api/checkout/state/order-confirmation`: p95 under `900ms`, starts payment/order flow, uses idempotency key, and returns a deterministic state response.
...
- Observability: Phase 1 handled HTTP responses have request ID and trace ID response headers plus structured completion logs with safe tenant, route, latency, and status context. Later instrumentation adds full OpenTelemetry metrics/traces and downstream gRPC timing.
...
- Error contract: public HTTP APIs return RFC 9457 `application/problem+json` for errors, with stable problem `type`, `title`, `status`, `detail`, `instance`, `traceId`, public `tenant`/`shop` slugs when safe, and field-level `errors` for validation.
...
Recommended local observability posture:
...
- No mandatory local observability backend in the default developer path.
- Optional exporter profiles may add Grafana Alloy, OpenTelemetry Collector, Datadog Agent, Dash0 OTLP export, or a self-hosted Grafana stack later.

Recommended deploy observability posture:

- Keep application instrumentation OpenTelemetry/OTLP-first.
...
- Telemetry Export Adapter: receives or forwards OTLP from services and exports to the selected backend without changing application instrumentation.
...
- Analytics events.
- Business metrics exported through the selected observability backend.
...
- Telemetry resource attributes and labels include `tenant`, `service`, `environment`, `checkout_step`, and `order_result`, independent of the selected backend.
...
- Keep telemetry OpenTelemetry/OTLP-compatible and choose any backend exporter through a later explicit profile.
...
- Observability must be OTLP-first; defer the backend choice until the observability slice.
...
- Add an explicit exporter profile for the chosen backend when the observability slice starts. Candidate profiles are Grafana Cloud, Datadog, Dash0, or a self-hosted Grafana stack.
- Add dashboards and SLO evidence for the chosen backend without coupling application code to it.
...
- Decide whether deploy mode uses direct OTLP export, a node-local collector/agent, or a collector gateway; avoid sidecars unless isolation is required.
